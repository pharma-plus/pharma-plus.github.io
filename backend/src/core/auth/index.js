...
// Identifiant de connexion : adresse e-mail OU nom d'utilisateur (3-40 car. alphanum. + ._-)
const identifierSchema = Joi.alternatives().try(
  Joi.string().email().max(254),
  Joi.string().pattern(/^[a-zA-Z0-9._-]{3,40}$/),
);
...
router.post('/login', loginLimiter, validate({
  body: Joi.object({
    email: identifierSchema.required(),
